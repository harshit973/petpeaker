 ...
 */
@WebServlet("/admin/breed/highlights")
public class adminbreedhighlights extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			if(request.getSession().getAttribute("staffdata")!=null && ((HashMap)request.getSession().getAttribute("staffdata")).get("user")!=null && ((HashMap)request.getSession().getAttribute("staffdata")).get("user").equals(true) && new accesscheck(((HashMap)request.getSession().getAttribute("staffdata"))).isaccessable(request.getRequestURI().toString())) {		
				PreparedStatement ps=new database().connect().prepareStatement("select bh.id,br.breed,bh.highlight from `breedhighlights` bh inner join breeds br on br.id=bh.breed");
				...
				data.put("vals", vals);
				data.put("cols", new String[] {"breed","highlights"});
				System.out.print(request.getRequestURI());
				request.setAttribute("modal","Breed highlight");			
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {		
			...
			for(String i:data) {
				PreparedStatement ps=new database().connect().prepareStatement("delete from breedhighlights where id=?");
				ps.setString(1,i);
				ps.execute();
			}

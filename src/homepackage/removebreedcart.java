 ...
 */
@WebServlet("/removebreedcart")
public class removebreedcart extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			if( request.getSession().getAttribute("userdata")!=null && ((HashMap)request.getSession().getAttribute("userdata")).get("user").equals(true) ) {
				PreparedStatement ps=new database().connect().prepareStatement("delete from breedcart where uid=? and id=?");			
				ps.setString(1,String.valueOf(((HashMap)request.getSession().getAttribute("userdata")).get("uid")));
				ps.setString(2,String.valueOf(request.getParameter("bid")));			
				ps.execute();

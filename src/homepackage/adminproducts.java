...
public class adminproducts extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			PreparedStatement ps=new database().connect().prepareStatement("select id,category,brand,pname,thumbnail,price,rating,note,qty from `admin-product`");
			...
			data.put("vals", vals);
			data.put("cols", new String[] {"category","brand","product","thumbnail","price","rating","note","stock"});
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {		
			...
			for(String i:data) {
				PreparedStatement ps=new database().connect().prepareStatement("delete from products where id=?");
				ps.setString(1,i);
				ps.execute();
			}

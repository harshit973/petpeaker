 ...
 */
@WebServlet("/admin/products/flavor/change")
public class adminproductsflavorchange extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			PreparedStatement ps2=new database().connect().prepareStatement("select * from flavor where id=?");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			PreparedStatement ps=new database().connect().prepareStatement("update flavor set pid=?,flavor=? where id=?");
			String[] names=new String[] {"pdata","flavor","eid"};
			int i=1;
			for(String j:names) {

 ...
 */
@WebServlet("/admin/breeds")
public class adminbreeds extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			PreparedStatement ps=new database().connect().prepareStatement("select id,pet,breed,thumbnail,gender,price,stock from `admin-breed`");
			...
			while(rs.next()) {
				...
				d[2]=rs.getString(3);
				d[3]=rs.getString(4);				
				d[4]=(rs.getString(5).equals("0"))?"Male":"Female";								
				d[5]=rs.getString(6);								
				d[6]=rs.getString(7);												
				vals.add(d);
			}
			...
			data.put("vals", vals);
			data.put("cols", new String[] {"Pet","breed","thumbnail","gender","price","stock"});
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {		
			...
			for(String i:data) {
				PreparedStatement ps=new database().connect().prepareStatement("delete from breeds where id=?");
				ps.setString(1,i);
				ps.execute();
			}

 ...
 */
@WebServlet("/admin/products/size/change")
public class adminproductssizechange extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			PreparedStatement ps2=new database().connect().prepareStatement("select * from size where id=?");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			PreparedStatement ps=new database().connect().prepareStatement("update size set pid=?,size=? where id=?");
			String[] names=new String[] {"pdata","size","eid"};
			int i=1;
			for(String j:names) {

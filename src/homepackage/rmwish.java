...
/**
 * Servlet implementation class rmwish
 */
@WebServlet("/rmwish")
public class rmwish extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			if( request.getSession().getAttribute("userdata")!=null && ((HashMap)request.getSession().getAttribute("userdata")).get("user").equals(true) ) {
				PreparedStatement ps=new database().connect().prepareStatement("delete from whishlist where uid=? and pid=?");			
				...
				ps.execute();
				response.sendRedirect("/account?wishlist=1");
			}else {

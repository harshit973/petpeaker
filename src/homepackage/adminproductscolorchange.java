 ...
 */
@WebServlet("/admin/products/color/change")
public class adminproductscolorchange extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			PreparedStatement ps2=new database().connect().prepareStatement("select * from color where id=?");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			PreparedStatement ps=new database().connect().prepareStatement("update color set pid=?,color=? where id=?");
			String[] names=new String[] {"pdata","color","eid"};
			int i=1;
			for(String j:names) {

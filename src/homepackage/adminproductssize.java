 ...
 */
@WebServlet("/admin/products/size")
public class adminproductssize extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			PreparedStatement ps=new database().connect().prepareStatement("select s.id as `id`,pd.pname,s.size from size s inner join products pd on s.pid=pd.id");
			...
			data.put("vals", vals);
			data.put("cols", new String[] {"product","size"});
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {		
			...
			for(String i:data) {
				PreparedStatement ps=new database().connect().prepareStatement("delete from size where id=?");
				ps.setString(1,i);
				ps.execute();
			}

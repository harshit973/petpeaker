 ...
 */
@WebServlet("/admin/product/highlights")
public class adminproducthighlights extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			if(request.getSession().getAttribute("staffdata")!=null && ((HashMap)request.getSession().getAttribute("staffdata")).get("user")!=null && ((HashMap)request.getSession().getAttribute("staffdata")).get("user").equals(true) && new accesscheck(((HashMap)request.getSession().getAttribute("staffdata"))).isaccessable(request.getRequestURI().toString())) {		
				PreparedStatement ps=new database().connect().prepareStatement("select bh.id,br.pname,bh.highlight from `highlights` bh inner join products br on bh.pid=br.id");
				...
				data.put("vals", vals);
				data.put("cols", new String[] {"product","highlights"});
				request.setAttribute("modal","Product highlight");			
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {		
			...
			for(String i:data) {
				PreparedStatement ps=new database().connect().prepareStatement("delete from highlights where id=?");
				ps.setString(1,i);
				ps.execute();
			}

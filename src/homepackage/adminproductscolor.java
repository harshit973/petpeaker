 ...
 */
@WebServlet("/admin/products/color")
public class adminproductscolor extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			PreparedStatement ps=new database().connect().prepareStatement("select c.id as `id`,pd.pname,c.color from color c inner join products pd on c.pid=pd.id");
			...
			data.put("vals", vals);
			data.put("cols", new String[] {"product","color"});
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {		
			...
			for(String i:data) {
				PreparedStatement ps=new database().connect().prepareStatement("delete from color where id=?");
				ps.setString(1,i);
				ps.execute();
			}
